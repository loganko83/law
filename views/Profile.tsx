...
import { User, Settings, CreditCard, LogOut, LogIn, Shield, Mail, Phone, ChevronRight, FileClock, History, Edit2, Calendar, X, Check, Trash2, Plus, Smartphone, Database, Brain, Sun, Moon, Fingerprint, Copy, AlertCircle, CheckCircle, Loader } from 'lucide-react';
...
export const Profile: React.FC<ProfileProps> = ({ userProfile, onUpdateProfile, onBack, onLogin, onNavigateToBilling, onNavigateToDevPortal }) => {
  ...
  const truncateDidAddress = (address: string) => {
      ...
      <AnimatePresence>
        {isRevokeModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <div className="flex justify-between items-center mb-6">
                <h3 className="font-bold text-lg text-slate-800">{t('did.revokeConfirmTitle')}</h3>
                <button onClick={() => setIsRevokeModalOpen(false)} className="text-slate-400 hover:text-slate-600">
                  <X size={20} />
                </button>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 mb-1.5">{t('did.revokeReasonLabel')}</label>
                  <textarea
                    value={revokeReason}
